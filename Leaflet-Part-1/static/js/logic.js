...
//store the queryUrl for the GeoJSON data
...
d3.json(queryUrl).then(function (data) { //pull the earthquake JSON data with d3
    L.geoJson(data, {
    ...
    //this function pulls the tectonic plate data and draws an orange line over the plates
...
d3.json(queryUrl).then(function (data) {
    ...
    let results = features.filter(id => id.id == "nc74015746"); //replace the id string with the argument of the function once created
    let first_result = results[0];
    console.log(first_result);
    let geometry = first_result.geometry;
    //let geometry = first_result.properties.place;
